module.exports = function (creeps) {
    var loop = function () {
            ...
            if (creep.carry.energy == 0 || (Memory.creeps[creep.name].state == "empty" && creep.carry.energy < creep.carry.energyCapacity)) {
                ...
                if (t) {
                    if (creep.pos.getRangeTo(Game.getObjectById(t.id)) > 1) {
                        ...
                        if (!!creep.transfer(Game.getObjectById(t.id), RESOURCE_ENERGY)) {
                            Memory.creeps[creep.name].target = false;
                        }
                        ;
                    }
                    ...
                    for (var i in Game.spawns) {
                        ...
                        var countTrans = 0;
                        for (var i in Game.creeps) {
                            if (Memory.creeps[Game.creeps[i].name].role == "trans" && Game.creeps[i].room.name == spawn.room.name) {
                    ...
                    for (var i in Game.creeps) {
                        ...
                        var countTrans = 0;
                        for (var i in Game.creeps) {
                            if (Memory.creeps[Game.creeps[i].name].role == "trans" && Game.creeps[i].room.name == creep2.room.name) {
                                countTrans = countTrans + 1;
                            }
                            ;
                        }
                    ...
                    var string = "";
                    for (var i in target) {
                        string = string + Math.round(target[i].score * 100) / 100 + ", ";
                    }
                    ;
                ...
                if (creep.carry.energy > creep.carry.energyCapacity * 0.8) {
                    for (var i in Game.creeps) {
                        if (Memory.creeps[Game.creeps[i].name].role == "contr") {
                            if (Game.creeps[i].energy < Game.creeps[i].energyCapacity * 0.8 && !creep.transferEnergy(Game.creeps[i], creep.carry.energy / 2)) {
                                continue;
                            }
                            ;
                        }
        ...
        ;
        if (pulse) {
            pipes.safe = findSources(Game.spawns.Spawn1.room)
        }
        ;
        for (var i in Game.spawns) {
            var spawn = Game.spawns[i];
            if (!spawn.spawning) {
                ...
                for (var creep in Game.creeps) {
                    if (Memory.creeps[Game.creeps[creep].name].role == "contr" && Game.creeps[creep].room.name == spawn.room.name) {
                        countContr++;
                    }
                    if (Memory.creeps[Game.creeps[creep].name].role == "trans") {
                        countTransAll = countTransAll + 1;
                    }
                    if (Memory.creeps[Game.creeps[creep].name].role == "trans" && Game.creeps[creep].room.name == spawn.room.name) {
                        countTrans = countTrans + 1;
                    }
                    if (Memory.creeps[Game.creeps[creep].name].role == "miner" && Game.creeps[creep].room.name == spawn.room.name) {
                        countMiners = countMiners + 1;
                    }
                }
                if (countMiners > countTrans) {
                    console.log("Create Trans:", Game.spawns[i].createCreep([MOVE, MOVE, MOVE, CARRY, CARRY, CARRY], "trans" + getID(), {
                        role: "trans",
                        target: false
                    }));
                }
                for (var id in Memory.rooms[spawn.room.name].sources) {
                    ...
                    var count = 0;
                    for (var creep in Game.creeps) {
                        if (Memory.creeps[Game.creeps[creep].name].target && Memory.creeps[Game.creeps[creep].name].target == id) {
                            count++;
                        }
                    }
                    // don't let there be more than double the miners than transports
                    if (countMiners < 1) {
                        console.log("Create m1ner:", Game.spawns[i].createCreep([MOVE, WORK], "miner" + getID(), {
                            role: "miner",
                            target: id
                        }));
                    }
                    if (count < Math.floor(source.spots / 2)) {
                        console.log("Create m2ner:", Game.spawns[i].createCreep([MOVE, WORK, WORK], "miner" + getID(), {
                            role: "miner",
                            target: id
                        }));
                    }
                    ...
                    console.log("remaining spots (" + id + "): " + (source.spots - count));
                }
                ;
                if (countMiners > 1 && countTrans < 7 && countTransAll < _.size(Game.spawns) * 8) {
                    console.log("Create Trans:", Game.spawns[i].createCreep([MOVE, MOVE, MOVE, CARRY, CARRY, CARRY], "trans" + getID(), {
                        role: "trans",
                        target: false
                    }));
                ...
                ;
                if (countMiners > 1 && Memory.rooms[spawn.room.name].contr && countContr < Memory.rooms[spawn.room.name].contr[spawn.room.controller.id].spots.length) {
                    console.log("Create Contr:", Game.spawns[i].createCreep([MOVE, CARRY, WORK, WORK], "contr" + getID(), {
                        role: "contr",
                        target: contr
                    }));
                }
        ...
        for (var i in Game.creeps) {
            ...
            else if (Memory.creeps[creep.name].role == "trans") {
                roleTrans(creep)
            }
            else if (Memory.creeps[creep.name].role == "contr") {
                roleContr(creep)
            }
        ...
        var hostilesCount = {};
        for (var i in Game.spawns) {
            Game.spawns[i].room.find(FIND_HOSTILE_CREEPS, {
                filter: function (i) {
